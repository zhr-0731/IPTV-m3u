...
def main():
    global stats
    print(f"[源1] 获取远程 M3U 文件: {M3U_URL}")
    ...
    print(f"[源1] 共 {stats['total']} 个频道")
    
    target_entries = [e for e in all_entries if e["group"] in TARGET_GROUPS]
    stats["target"] = len(target_entries)
    print(f"[源1] 符合 group-title 要求的频道: {stats['target']} 个")
    
    if not target_entries:
        print("[源1] 没有符合条件的频道，退出。")
    ...
    print(f"[源1] 可用频道: {stats['valid']} 个")
    
    if not valid_entries:
        print("[源1] 没有可用的新频道，退出。")
        with open(".stats_iptv4.txt", "w", encoding="utf-8") as f:
    ...
    print(f"[源1] 需要新增的频道: {stats['new']} 个")
